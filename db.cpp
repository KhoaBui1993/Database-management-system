...
int sem_insert_table(token_list *t_list)
{
	...
	bool report = false;
	int i = 0;
	tpd_entry *table_entry;
	table_entry = (tpd_entry*)malloc(sizeof(table_entry));
	memset(table_entry, '\0', sizeof(tpd_entry));

	//iterate through t_list
	cur = t_list;
	token_list *column_tokens = NULL;
	token_list *temp = column_tokens;

	while (cur != NULL)
	{
		switch (cur->tok_class)
		{
		case /* constant-expression */5:
			if (temp == NULL) {
				temp = (token_list*)malloc(sizeof(token_list));
				memset(temp, '\0', sizeof(token_list));
				...
				token_list *temp2 = (token_list*)malloc(sizeof(token_list));
				memset(temp, '\0', sizeof(token_list));
				temp->next = temp2;
				strcpy(temp2->tok_string, cur->tok_string);
				temp2->tok_class = cur->tok_class;
				temp2->tok_value = cur->tok_value;
				temp2->next = NULL;	
				temp = temp2;
			}
	...
	strcpy(table_entry->table_name,cur->tok_string);
	strcpy(tablename, table_entry->table_name);
	strcpy(filename, strcat(table_entry->table_name, ".tab"));
	...
	else
	{
		...
		old_header->tpd_ptr = get_tpd_from_list(table_entry->table_name);

		//func
		if (old_header->tpd_ptr){
			cur = column_tokens;
			tpd_entry *tab = old_header->tpd_ptr;
			int num_of_col = tab->num_columns;
			int offset = tab->cd_offset;
			cd_entry *col;
			for (int i = 0; i < num_of_col; i++) {
				if (cur != NULL)
					col = (cd_entry*)(tab + tab->cd_offset + i * sizeof(cd_entry));
				else {

				}
				//compare col->type vs cur->value
				//
				cur = cur->next;
			}
			if (cur != NULL){
			...
			for (int i = 0; i < num_of_col; i++) {
				...
				col = (cd_entry*)(tab + tab->cd_offset + i * sizeof(cd_entry));
				//compare col->type vs cur->value
				fwrite(cur->tok_string, col->col_len, 1, fhandle);
				cur = cur->next;
			}
		}
		else
		...
		//printf("%p",old_header->tpd_ptr);
		printf("%d",old_header->record_offset);
		fclose(fhandle);
	//rc = add_row_to_file(old_header, column_tokens);
	if (rc == 0)
	{
		printf("row inserted successfully.\n");
		old_header->tpd_ptr = NULL;
		free(old_header);
		free(fhandle);
	}
	else
...
int add_row_to_file(table_file_header *old_header, token_list *t_list)
{
...
//add code to create <table_name>.tab
if (rc==0)
{
	cur=t_list;
	int record_size=0;
	...
	while (cur != NULL)
	{
		...
		case T_CHAR:
			cur = cur->next->next;
			if (cur->tok_value == INT_LITERAL){
				record_size += 1 + atoi(cur->tok_string);
			}
			break;
		case T_VARCHAR:
			cur = cur->next->next;
	...
	if (record_size % 4 != 0){
		int remainder = record_size%4;
		record_size = record_size + (4-remainder);
	}
	cur = t_list; 
	FILE *fhandle = NULL;
//	struct _stat file_stat;
	...
	if((fhandle = fopen(file_name,"rbc")) == NULL)
	{
		...
		{
			table_file_header* tab_header = (table_file_header *)malloc(sizeof(table_file_header));
			memset(tab_header,0,sizeof(table_file_header));
			...
			printf ("Total size: %d \n",tab_header->file_size+tab_header->record_size*100);
			printf ("filename: %s \n",file_name);
			free(tab_header);
			free(fhandle);
		}
		...
		else
		{
			...
			else
			{
				...
				if (rc==0)
				{
					...
					{
						printf("delete file sucessfully :%s" ,filename);
					}
					else
					rc = FILE_OPEN_ERROR;
				}
